...
elasticNetFit <- glmnet(xtrain, ytrain, alpha = 0.5, lambda = lambda.array, family = "binomial")

# Plot coefficients against lambda
plot(elasticNetFit, xvar = 'lambda', label = TRUE)

# Plot deviance explained
plot(elasticNetFit, xvar = 'dev', label = TRUE)

# Cross-validation for lambda selection
cv.elastic <- cv.glmnet(xtrain, ytrain, alpha = 0.5, family = "binomial")
best_lambda <- cv.elastic$lambda.min

# Predictions
y_predicted <- predict(elasticNetFit, s = best_lambda, newx = xtest, type = "response")
...
coefficients <- coef(elasticNetFit, s = best_lambda)
